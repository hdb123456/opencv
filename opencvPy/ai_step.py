...
def ai_move(board, ai_piece):
    ...
    if board[center_cell_pos[0]][center_cell_pos[1]] == EMPTY:
        ...
        save_game_state(center_cell_number, (world_x, world_y), board)
    ...
    if move:
        ...
        put_piece((row, col), ai_piece)
        save_game_state(cell_number, (world_x, world_y), board)
...
def save_game_state(position, world_coordinates, board_state, filename='read_file.json'):
    """
    保存游戏状态到 JSON 文件。
    
    :param position: 当前棋子的格子位置 (1-9)
    :param world_coordinates: 当前棋子的世界坐标 (x, y)
    :param board_state: 棋盘状态 (3x3 矩阵)
    :param filename: JSON 文件名
    """
    data = {
        "位置": position,
        "世界坐标": world_coordinates,
        "棋盘状态": board_state
    }
    # 将数据转换为 JSON 字符串
    ...
    frame_header = "AA\n"
    frame_footer = "\nBB"
    full_data = f"{frame_header}{json_data}{frame_footer}"
